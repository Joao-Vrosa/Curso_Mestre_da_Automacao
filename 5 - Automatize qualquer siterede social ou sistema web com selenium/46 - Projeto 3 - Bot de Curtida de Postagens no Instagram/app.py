...
username = wait.until(condicao_esperada.element_to_be_clickable((By.XPATH, "//input[@name='username']")))
username.send_keys("USUARIO")

sleep(2)

# - Clicar e digitar a senha
password = wait.until(condicao_esperada.element_to_be_clickable((By.XPATH, "//input[@name='password']")))
password.send_keys("SENHA")

sleep(3)

# - Clicar em entrar
entrar = wait.until(condicao_esperada.element_to_be_clickable((By.XPATH, "//div[contains(text(),'Entrar')] ")))
entrar.click()
...
while True:
    ...
    # - Verificar se postagem foi curtida, caso não tenha sido, clicar em curtir, caso ja tenha sido, aguardar 24 horas
    elementos_postagens = wait.until(condicao_esperada.visibility_of_any_elements_located((By.XPATH, "//div[@class='x6s0dn4 x78zum5 xdt5ytf xl56j7k']")))

    if len(elementos_postagens) == 8:
        elementos_postagens[0].click()
    else:
